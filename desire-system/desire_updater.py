...
DB_PATH = Path(os.getenv(
    "MEMORY_DB_PATH",
    str(Path.home() / ".gemini" / "memories" / "memory.db"),
))
...
COMPANION_NAME = os.getenv("COMPANION_NAME", "コウタ")
...
def get_latest_memory_timestamp(
    ...
    """
    # 最後に保存された記憶から順に検索
    query = "SELECT timestamp, content FROM memories ORDER BY timestamp DESC LIMIT 100"
    try:
        cursor = conn.execute(query)
        rows = cursor.fetchall()
    except Exception as e:
        print(f"[desire-updater] SQLiteクエリ実行エラー: {e}")
        return None
...
def main() -> None:
    ...
    try:
        ...
        print(f"[desire-updater] SQLite接続エラー ({DB_PATH}): {e}")
        return
