...
|----------|---------|-------------|
| `MEMORY_DB_PATH` | `~/.gemini/memories/memory.db` | SQLite database path |

## Migrating from ChromaDB

If you have existing memories in ChromaDB (older versions used `~/.gemini/memories/chroma`), run the migration script:
