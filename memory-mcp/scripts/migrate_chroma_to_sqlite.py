...
Usage:
    uv run python scripts/migrate_chroma_to_sqlite.py \
        --source ~/.gemini/memories/chroma \
        --dest ~/.gemini/memories/memory.db
...
def main() -> None:
    ...
    parser.add_argument(
        "--source",
        default=str(Path.home() / ".gemini" / "memories" / "chroma"),
        help="Path to ChromaDB directory (default: ~/.gemini/memories/chroma)",
    )
    parser.add_argument(
        "--dest",
        default=str(Path.home() / ".gemini" / "memories" / "memory.db"),
        help="Path to SQLite output file (default: ~/.gemini/memories/memory.db)",
    )
